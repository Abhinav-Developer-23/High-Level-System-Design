...
During a major festival period, transaction volume on our UPI platform increased significantly. We had already performed capacity planning and scaled our services based on expected traffic projections.

However, actual traffic exceeded our estimates by a large margin.

As traffic kept increasing, our transaction processing service started experiencing resource exhaustion. Several pods became unhealthy and started crashing. New pods were also struggling to come up because they immediately received overwhelming traffic after startup.

Very quickly, customers started facing transaction failures and delays. Complaints began appearing on LinkedIn, Twitter, and customer support channels.

Because payments were getting impacted during a peak business period, this became a high-priority production incident.

**Task**

I was part of the core team responsible for restoring service availability as quickly as possible while minimizing customer impact.

My responsibility was to help identify the bottleneck, stabilize the system, and safely bring the service back online.

**Action**

We immediately created an incident bridge and started analyzing service metrics, pod health, CPU utilization, memory consumption, and request throughput.

After investigating, we discovered that the issue was not a software bug but a capacity problem. The incoming traffic was significantly higher than the number of pods could safely handle.

Additionally, whenever Kubernetes attempted to start new pods, they were flooded with requests immediately, preventing them from becoming healthy and serving traffic effectively.

To stabilize the system, we made the decision to temporarily stop incoming traffic to the affected service.

While traffic was blocked:

* We increased the pod count substantially.
* Verified infrastructure capacity.
* Monitored pod startup and health checks.
* Ensured all pods became stable before accepting traffic.
* Conducted controlled validation using internal traffic.

Once the environment was stable, we gradually released traffic instead of allowing a sudden spike.

Throughout the incident, we continuously communicated updates to stakeholders and support teams.

**Result**

The service was fully restored after approximately 30 minutes.

Transaction processing returned to normal levels, and customer complaints started decreasing shortly afterward.

More importantly, we treated the incident as a learning opportunity rather than simply closing it.

**What I Learned / Improvements**

During the postmortem, we identified that scaling infrastructure alone was not sufficient. We also needed better protection mechanisms at the application layer.

As a result, I helped drive the implementation of:

* Per-pod rate limiting.
* Better traffic throttling controls.
* Improved autoscaling thresholds.
* More aggressive alerting before saturation points were reached.
* Load testing using higher traffic assumptions than previous festival projections.

In subsequent high-traffic events, these improvements prevented pods from being overwhelmed and significantly improved platform stability.

**Why This Was Critical**

The issue directly impacted customer payments during one of the highest traffic periods of the year. Every minute of downtime affected customer trust and business metrics, so restoring the service quickly and preventing recurrence was extremely important.
